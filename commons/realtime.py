...
def analysis(db_path, model_name = 'VGG-Face', detector_backend = 'opencv', distance_metric = 'cosine', enable_face_analysis = True, enable_face_detection=False,source = 0, time_threshold = 5, frame_threshold = 5, call_back_func=None):
	...
	while(True):
		...
		# cv2.setWindowProperty('img', cv2.WND_PROP_AUTOSIZE, cv2.WINDOW_NORMAL)
				...
				if freezed_frame == 0:
					...
					for detected_face in detected_faces_final:
						...
						if enable_face_analysis == True:
							...
							if call_back_func!=None:
								call_back_func(emotion_df,apparent_age,gender,time.time()-start_time)
